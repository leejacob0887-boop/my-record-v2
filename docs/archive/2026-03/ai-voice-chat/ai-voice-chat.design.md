...
// useSpeechInput 훅 (handleSendRef 패턴 — 섹션 7 참고)
const handleSendRef = useRef<() => void>(() => {});
// handleSend 정의 후 useEffect로 최신 참조 유지
const handleVoiceResult = useCallback((text: string) => {
  setInput(text);
  setTimeout(() => handleSendRef.current(), 0); // deps 없이 안정적 참조
}, []);
...
| 2026-03-12 | 구현 완료 — 실제 코드 기반 상태 설계 보정, 수용 기준 전체 달성 | Claude |
