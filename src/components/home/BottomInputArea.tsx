...
export default function BottomInputArea({
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 max-w-[430px] mx-auto bg-white dark:bg-gray-800 rounded-t-3xl shadow-[0_-4px_20px_rgba(0,0,0,0.08)] px-4 pt-4 pb-8 space-y-3 z-30">
      <QuickButtons onSelect={onQuickButton} />
      {/* 입력창 */}
      <div className="flex items-center gap-2 bg-[#F4F2EE] dark:bg-gray-700 rounded-3xl px-4 py-3 shadow-inner">
        <button className="text-gray-400 dark:text-gray-500 flex-shrink-0" aria-label="추가">
          <Plus size={20} />
        </button>
        <button
          onClick={onMic}
        ...
        <button
          ...
          className="w-9 h-9 rounded-full bg-[#0F6E56] flex items-center justify-center flex-shrink-0 disabled:opacity-40 active:opacity-70 transition-opacity"
          aria-label="전송"
        >
