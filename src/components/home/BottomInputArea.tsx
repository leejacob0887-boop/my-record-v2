...
export default function BottomInputArea({
  ...
  return (
    ...
    >
      ...
      {/* 입력창 */}
      {/* form autoComplete="off" — 삼성 패스 툴바 억제 시도 */}
      <form autoComplete="off" onSubmit={(e) => { e.preventDefault(); onSend(); }}
        className="flex items-center gap-2 bg-[#F4F2EE] dark:bg-gray-700 rounded-3xl px-3 py-2.5 shadow-inner overflow-hidden">
        <button type="button" className="text-gray-400 dark:text-gray-500 flex-shrink-0 p-1" aria-label="추가">
          <Plus size={20} />
        </button>
        <button
          type="button"
