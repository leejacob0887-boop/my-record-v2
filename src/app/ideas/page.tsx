...
function SkeletonList() {
  return (
    <div className="animate-pulse flex flex-col gap-3">
      {[...Array(4)].map((_, i) => (
        <div key={i} className="bg-white dark:bg-gray-800 border border-green-200 dark:border-green-900 rounded-2xl p-4 h-20" />
      ))}
    </div>
  );
}
...
function IdeaCard({
  ...
  const body = (
    <>
      {editMode ? (
        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${isSelected ? 'bg-green-600 border-green-600' : 'border-gray-300 dark:border-gray-500'}`}>
          {isSelected && (
      ...
      <div className="flex-1 min-w-0">
        <p className="text-base font-semibold text-gray-800 dark:text-gray-100 truncate">{idea.title}</p>
        <p className="text-sm text-green-500 dark:text-green-400 mt-0.5">{formatDateTime(idea.date ?? idea.createdAt.slice(0, 10), idea.createdAt)}</p>
        {idea.content && (
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5 truncate">{idea.content}</p>
        )}
      </div>
      {!editMode && <ChevronRight size={18} color="#86EFAC" className="flex-shrink-0" />}
    </>
  );

  if (editMode) {
    return (
      <div onClick={() => idea.id && onToggle(idea.id)} className={`cursor-pointer ${cardClass}`}>
        {body}
      </div>
    );
  }
  ...
  return (
    <Link href={`/ideas/${idea.id}`} className={cardClass}>
      {body}
    </Link>
  );
}
...
export default function IdeasPage() {
  ...
  const toggleSelectAll = () => {
    if (allSelected) {
      ...
      setSelected(new Set(filtered.map(i => i.id).filter((id): id is string => Boolean(id))));
    }
  };
    ...
    <main className="min-h-screen bg-[#F4F2EE] dark:bg-gray-900">
      <div className="max-w-[430px] mx-auto px-4 pb-8">
        ...
        <div className="relative flex items-center justify-center mb-6">
          {editMode && (
            <button onClick={toggleSelectAll} className="absolute left-0 flex items-center gap-1.5 py-1 px-1">
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${allSelected ? 'bg-green-600 border-green-600' : 'border-gray-300 dark:border-gray-500'}`}>
                {allSelected && (
          ...
          {ideas.length > 0 && (
            <div className="absolute right-0 flex items-center gap-1">
              ...
              <button
                ...
                className="text-sm font-medium text-green-600 py-1 px-2"
              >
                {editMode ? '취소' : '편집'}
              </button>
            </div>
        ...
        {!editMode && allTags.length > 0 && (
          <div className="flex gap-2 overflow-x-auto pb-1 mb-4" style={{ scrollbarWidth: 'none' }}>
            <button
              ...
              className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${tagFilter === null ? 'bg-green-600 text-white' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400'}`}
            >
              전체
            </button>
            {allTags.map(tag => (
              <button
                ...
                className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${tagFilter === tag ? 'bg-green-600 text-white' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400'}`}
              >
                #{tag}
              </button>
            ))}
        ...
        {!editMode && (
          <Link
            href="/ideas/new"
            className="flex items-center justify-center gap-2 w-full bg-green-600 hover:bg-green-700 text-white rounded-2xl py-3.5 text-base font-semibold transition-colors mb-5"
          >
            + 새 아이디어
          </Link>
        )}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            <p className="text-gray-400 text-base">아직 아이디어가 없어요</p>
            <p className="text-gray-300 dark:text-gray-600 text-sm mt-1">첫 번째 아이디어를 기록해보세요</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-16">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            <div className="w-full max-w-[360px] bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
              <div className="px-6 py-5">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">선택한 {selected.size}개의 아이디어를 삭제할까요?</p>
