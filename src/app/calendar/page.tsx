...
import { getTodos, toggleTodo, type Todo } from '@/lib/todos'
...
export default function CalendarPage() {
  ...
  // 이번 달 일정 불러오기
  const fetchMonthEvents = useCallback(async (y: number, m: number) => {
    try {
      const events = await getEventsByMonth(y, m + 1) // m is 0-indexed
      setMonthEvents(events)
    } catch (e) {
      console.error('[CalendarPage] fetchMonthEvents error', e)
    }
  }, [])
  ...
  // 투두 전체 불러오기 (마감일 필터링용)
  useEffect(() => {
    getTodos().then(setAllTodos).catch(console.error)
  }, [])
  ...
  async function handleSelectDate(dateStr: string) {
    ...
    try {
      const [events, todos] = await Promise.all([
        ...
        Promise.resolve(allTodos.filter((t) => t.due_date === dateStr)),
      ])
      setDayEvents(events)
      setDayTodos(todos)
      setShowDaySheet(true)
    } catch (e) {
      console.error('[CalendarPage] handleSelectDate error', e)
    }
  }
  ...
  async function handleSaveEvent(data: {
    ...
    if (editingEvent) {
      ...
      setDayEvents((prev) => prev.map((e) => (e.id === updated.id ? updated : e)))
      setMonthEvents((prev) => prev.map((e) => (e.id === updated.id ? updated : e)))
    } else {
      const created = await addEvent(data)
      if (created.date === selectedDate) {
        setDayEvents((prev) => [...prev, created])
      }
      setMonthEvents((prev) => [...prev, created])
    }
  ...
  async function handleToggleTodo(id: string, is_done: boolean) {
    setDayTodos((prev) => prev.map((t) => (t.id === id ? { ...t, is_done } : t)))
    try {
      await toggleTodo(id, is_done)
    } catch {
      setDayTodos((prev) => prev.map((t) => (t.id === id ? { ...t, is_done: !is_done } : t)))
    }
  }
  ...
  return (
    <main className="min-h-[100dvh] bg-[#FAF8F4] dark:bg-gray-900">
      <div className="max-w-[430px] mx-auto px-4 pb-24">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={goToToday} className="text-xs text-[#0F6E56] font-medium hover:opacity-70 transition-opacity">
            오늘
          </button>
        </div>
              ...
              return (
                <button
                  ...
                  className={`aspect-square flex flex-col items-center justify-center gap-0.5 transition-colors rounded-xl
                    ${isToday ? 'bg-[#0F6E56] text-white' : 'hover:bg-gray-50 dark:hover:bg-gray-700'}
                ...
                >
                  ...
                  <span className="flex gap-0.5 h-1.5 items-center">
                    {hasEvent && (
                      <span className={`w-1 h-1 rounded-full ${isToday ? 'bg-white/80' : 'bg-[#0F6E56]'}`} />
                    )}
                    {hasRecord && (
                      <span className={`w-1 h-1 rounded-full ${isToday ? 'bg-white/60' : 'bg-[#4A90D9]'}`} />
                    )}
                  </span>
                </button>
              )
        ...
        {(() => {
          ...
          return (
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm px-5 py-4">
              ...
              {total === 0 ? (
                <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-1">오늘 아직 기록이 없어요 🌱</p>
              ) : (
      ...
      {(summaryText || summaryError) && (
        <>
          <div className="fixed inset-0 bg-black/40 backdrop-blur-[2px] z-50" onClick={closeSummary} />
            ...
            <div className="pointer-events-auto w-full max-w-[400px] bg-white dark:bg-gray-800 rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-700 overflow-hidden animate-slide-up-toast">
              ...
              <div className="px-6 py-5">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <button onClick={closeSummary} className="w-7 h-7 flex items-center justify-center rounded-full text-gray-300 hover:text-gray-500 hover:bg-gray-100 transition-colors flex-shrink-0 ml-3">
                ...
                {summaryText && <p className="text-sm text-gray-700 dark:text-gray-200 leading-relaxed whitespace-pre-wrap">{summaryText}</p>}
                {summaryError && <p className="text-sm text-red-500 dark:text-red-400">{summaryError}</p>}
                {summaryText && (
                  <div className="flex gap-2 mt-5">
                    <button onClick={closeSummary} className="flex-1 py-2.5 rounded-xl text-sm font-medium text-gray-500 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 transition-colors">닫기</button>
                    <button onClick={handleSaveSummary} className="flex-[1.5] py-2.5 rounded-xl text-sm font-semibold text-white transition-all active:scale-[0.98]" style={{ background: 'linear-gradient(135deg, #8B5CF6, #A855F7)' }}>메모로 저장 ✨</button>
                  </div>
                )}
              </div>
            </div>
