...
function MomentCard({
  ...
  const body = (
    <>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 truncate">{moment.text}</p>
  ...
  if (editMode) {
    return (
      <div
        onClick={() => moment.id && onToggle(moment.id)}
...
export default function MomentsPage() {
  ...
  const handleDeleteConfirm = () => {
    const idsToDelete = deleteTarget === 'all'
      ? filtered.map(m => m.id).filter((id): id is string => Boolean(id))
      : Array.from(selected);
      ...
      <div className={`max-w-[430px] mx-auto px-4 ${editMode ? 'pb-28' : ''}`}>
        ...
        <div className="relative flex items-center justify-center mb-6">
          <div className="text-center">
            <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100">메모</h1>
            ...
            <div className="w-full max-w-[360px] bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
              <div className="px-6 py-5">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">
                  {deleteTarget === 'all'
                    ? `${filtered.length}개의 메모를 모두 삭제할까요?`
                    : `선택한 ${selected.size}개의 메모를 삭제할까요?`}
                </p>
